...
RS_BIT16 crchware(RS_BIT16 data, RS_BIT16 genpoly, RS_BIT16 accum);

/* Computes the CRC-CCITT checksum on array of byte data, length len
*/
RS_BIT16 crc_ccitt(unsigned char *msg, int len)
{
	int i;
	RS_BIT16 acc = 0;

	for (i = 0; i < len; i++) {
		acc = crchware((RS_BIT16) msg[i], (RS_BIT16) 0x1021, acc);
	}
