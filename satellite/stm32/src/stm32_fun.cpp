...
// Global Objects
STM32RTC& rtc = STM32RTC::getInstance(); // L496 Internal RTC
...
void setUNIX(uint32_t unixTime)
{
    if (!rtc.isTimeSet())
{
        rtc.begin(); // Selects LSE (External 32k Crystal) automatically if available
    }

    if (unixTime == 0)
{
        // Default to Jan 1, 2025
        rtc.setEpoch(1735689600);
    }
	else
	{
...
{
    // 1. Create a local HMAC object
    SHA256 hmac;
    hmac.resetHMAC(SECRET_KEY, sizeof(SECRET_KEY));

    // 2. Prepare Header Data
    uint8_t header[PACKET_HEADER_LENGTH];
    header[0] = packet->station;
    header[1] = packet->ecc ? RS_ON : RS_OFF;
    header[2] = packet->command;
    header[3] = packet->payload_length;
    header[4] = (packet->time_unix >> 24) & 0xFF;
    header[5] = (packet->time_unix >> 16) & 0xFF;
    header[6] = (packet->time_unix >> 8)  & 0xFF;
    header[7] = (packet->time_unix)       & 0xFF;
    header[8] = 0; // MAC placeholder
    header[9] = 0;
    header[10] = 0;
    header[11] = 0;
    ...
    // 3. Feed Payload Data (if any)
    if (packet->payload_length > 0) {
        hmac.update(packet->payload, packet->payload_length);
    }

    // 4. Finalize HMAC
    uint8_t hash[32];
    hmac.finalizeHMAC(SECRET_KEY, sizeof(SECRET_KEY), hash, sizeof(hash));

    // 5. Truncate to 4 bytes (Big Endian)
    *out_mac = ((uint32_t)hash[0] << 24) |
               ((uint32_t)hash[1] << 16) |
               ((uint32_t)hash[2] << 8)  |
               ((uint32_t)hash[3]);
