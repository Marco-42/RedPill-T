...
// il modulo in INITIATING_NODE deve essere connesso ad una porta seriale
...
void setup() {
  ...
  // se il modulo viene definito come INITIATING_NODE allora questo inizia la comunicazione e viene trasmesso il primo pacchetto
    ...
    // Richiedo il pacchetto dalla porta seriale
    String utente;
    Serial.println("MESSAGGIO:  ");
    while (Serial.available()) 
      Serial.read();   
    while(Serial.available() == 0){}
      utente = Serial.readStringUntil('\n');  

    // Trasmetto il primo pacchetto
    stato_trasmissione = radio1.startTransmit(utente);
...
void loop() {
  ...
  if(check_packet) {
    ...
    if(check_com) {
      ...
      // se è il dispositivo iniziale allora mando a schermo il pacchetto ricevuto ed inizio la nuova trasmissione
      #if defined(INITIATING_NODE)
      if (state == RADIOLIB_ERR_NONE) {
      ...
      // attendo 3 secondi da una ricezione alla trasmissione successiva
      delay(3000);
      
      // effettuo la trasmissione successiva
      Serial.print(F("[SX1278] TRASMISSIONE"));
      String messaggio;
      Serial.println("MESSAGGIO:  ");
      while (Serial.available()) 
      ...
      stato_trasmissione = radio1.startTransmit(messaggio);
      check_com = true;

      // se è l'altro dispositivo allora ritrasmetto il pacchetto ricevuto
      #else
      ...
      if (state == RADIOLIB_ERR_NONE) {
        // ritrasmetto il pacchetto
      stato_trasmissione = radio1.startTransmit(str);

      }else if(state == RADIOLIB_ERR_CRC_MISMATCH){

        // ritrasmetto un messaggio d'errore
        str = "[SX1278] RICEZIONE FALLITA ";
        stato_trasmissione = radio1.startTransmit(str);
      }
