...
class MainWindow(QWidget):
	...
	def __init__(self):
		...
		self.queue_timer.timeout.connect(self.enable_add_to_queue_button)
	...
	# Initialize right panel with tabs and status
	def init_right_panel(self):	
		...
		self.main_layout.addLayout(right_col, 1)

		self.setLayout(self.main_layout)

	# Create Serial Console tab
	...
	def create_settings_tab(self):
		...
		settings_title_label = QLabel("GROUND STATION SETTINGS")
		settings_title_label.setAlignment(Qt.AlignCenter)
		settings_title_label.setStyleSheet("font-size: 24px; font-weight: bold;")
		...
		self.freq_input.setRange(400.0, 500.0)
		self.freq_input.setDecimals(3)
		self.freq_input.setValue(436.0)
		self.freq_input.setSuffix(" MHz")
		lora_config_layout.addRow("Frequency:", self.freq_input)

		self.bandwidth_input = QComboBox()
		self.bandwidth_input.addItems(["62.5", "125", "250", "500"])
		self.bandwidth_input.setCurrentText("125")
		lora_config_layout.addRow("Bandwidth:", self.bandwidth_input)

		self.sf_input = QSpinBox()
		self.sf_input.setRange(6, 12)
		self.sf_input.setValue(10)
		lora_config_layout.addRow("SF:", self.sf_input)

		self.cr_input = QSpinBox()
		self.cr_input.setRange(5, 8)
		self.cr_input.setValue(5)
		lora_config_layout.addRow("CR:", self.cr_input)

		self.power_input = QSpinBox()
		self.power_input.setRange(-9, 22)
		self.power_input.setValue(10)
		self.power_input.setSuffix(" dBm")
		lora_config_layout.addRow("Power:", self.power_input)

		self.apply_lora_settings_btn = QPushButton("Apply Settings")
		self.apply_lora_settings_btn.clicked.connect(self.send_lora_config_command)
		self.apply_lora_settings_btn.setEnabled(False)
		lora_config_layout.addRow(self.apply_lora_settings_btn)

		lora_config_group.setLayout(lora_config_layout)
		settings_tab_layout.addWidget(lora_config_group)
		settings_tab_layout.addItem(QSpacerItem(20, 40, QSizePolicy.Minimum, QSizePolicy.Expanding))
			...
			try:
				while self.serial_conn.in_waiting:
					...
					if line.startswith("Decoded:"):
						...
						try:
							# Example line: "RSSI: -113.75 SNR: 9.20 dF: 1.02"
							parts = line.split()

							# Extract and convert values
							rssi = float(parts[1])
							snr = float(parts[3])
							freq_shift = float(parts[5])
	...
	# Send radio settings command to the serial port
	def send_lora_config_command(self):
		freq = self.freq_input.value() # float (MHz)
		bw = float(self.bandwidth_input.currentText()) # float (kHz)
		sf = self.sf_input.value() # int
		cr = self.cr_input.value() # int
		power = self.power_input.value() # int (dBm)

		# Format the line expected by ESP32
		command_line = f"RADIO: {freq:.3f} {bw:.1f} {sf} {cr} {power}\n"
	...
	def get_color_for_status(self, status):
		...
		elif status == "RECEIVED TEC":
			bg_color = QColor(0, 176, 255)  # light blue
	...
	def update_packet_content_form(self):
		...
		for field in fields:
			...
			if widget_class in (QSpinBox, QDoubleSpinBox):
				...
				# Use setSuffix when supported
				if right_text:
					spinbox.setSuffix(" " + right_text)
				...
				# Use label container only if suffix can't be applied
				if right_text:
					container = QWidget()
	...
	def handle_reception(self, decoded_packet, packet_bytes):
		...
		# TER is received, it is a reply to a command sent
		if ter in TER_TASKS.values():
			...
			if ter == TER_ACK:
				if payload == [self.last_sent_tec]:
					...
					self.last_tec_status_description.setText(f"TEC {tec_sent_label} executed successfully")
					self.log_status(f"[INFO] ACK received after {elapsed:.2f} s")

				else:
					status = "INVALID ACK"
					self.last_tec_status_description.setText(f"TEC {tec_sent_label} seems to be executed, but ACK was invalid. PAYLOAD: {payload}")
				...
				if len(payload) == 2:
					...
					self.last_tec_status_description.setText(f"TEC {tec_sent_label} was not executed. ERROR: {status_msg}")
				...
				self.last_tec_status_description.setText(f"TEC {tec_sent_label} executed successfully, requested data received")
		...
		# TEC is received, it is a command and not a reply to a command sent
		elif ter in TEC_TASKS.values():
			tec = ter
			status = "RECEIVED TEC"
			ter_tec_label = get_task_label(TEC_TASKS, tec)
			self.last_tec_status_description.setText(f"Received TEC {ter_tec_label}")
