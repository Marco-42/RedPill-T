...
class MainWindow(QWidget):
    ...
    # Clear and remove all widgets from layout and memory
        ...
        if key == (0, "TLE update"):
            btn = QPushButton("Simula orbita")
            btn.clicked.connect(self._on_simula_clicked)
            self.packet_setup_layout.addRow("", btn)
            self.created_widgets[(type_index, task_name, "Simula")] = btn
    ...
    def _on_simula_clicked(self):
        widget = self.created_widgets.get((0, "TLE update", "TLE Data:"))
        if widget:
            tle_data = widget.toPlainText().strip().splitlines()
            if len(tle_data) >= 2:
                line1, line2 = tle_data[:2]
                try:
                    self.simula_orbita(line1, line2)
                except Exception as e:
                    QMessageBox.critical(self, "Errore simulazione", str(e))
            else:
                QMessageBox.warning(self, "Dati mancanti", "Inserisci due righe TLE.")
    ...
    def simula_orbita(self, line1, line2):
        ...
        self.log_status("INITIAL TIME: " + str(start_time) + " UTC")
        latitudes = []
        longitudes = []
        altitudes = []
        conta = 0
        ...
        for t in time_steps:
            ...
            if e == 0:
                ...
                if distance <= gs_range and ver == True:
                    self.log_status(f"\nCONNECTION: {conta}")
                    self.log_status(f"GS in range at {t} UTC")
                    ver = False
                    conta += 1
                elif distance > gs_range and ver == False: 
                    self.log_status(f"GS out of range at {t} UTC")
                    ver = True
            else:
