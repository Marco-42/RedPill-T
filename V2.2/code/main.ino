...
SX1278 radio1 = new Module(CS_PIN, DIO0_PIN, RESET_PIN, DIO1_PIN);
...
void RX_manager(void *parameter)
{
	...
	for(;;)
	{
		// Wait for packet to be received --> the task is paused until the packet is received
		ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
		Serial.println("Packet received...");

		// When the packet is received the task read it
...
// Main task
// The task is looking for a packet gived by serial monitor. When the packet is re
void serial_manager(void *parameter)
{
	const TickType_t xDelay = 100 / portTICK_PERIOD_MS; // convertion of a 100 ms delay in RTOS ticks
...
void setup()
{
	...
	int8_t state = radio1.begin(433.0, 125.0, 10, 8, 0x12, 10, 8, 0);
	...
	// TX manager task
	RTOS_TX_queue = xQueueCreate(5, 64); // 5 packets of 64 bytes max --> queue beetween task
	xTaskCreate(TX_manager, "TX manager", 2000, NULL, 3, &RTOS_TX_manager_handle); //--> trasmitting has an higher priority than receiving
